...
const createEmptyCoursesByDay = (): CourseInDay[] => [
  { dayNumber: 1, courses: [] },
  { dayNumber: 2, courses: [] },
  {
    dayNumber: 3,
...
export const getAllCoursesInDay = (
  timetable: Timetable,
  day: DayNumber
...
export const calculateTimetable = (courses: Course[]): Timetable => {
  return {
    coursesByDays: courses.reduce((acc, course) => {
      ...
      acc[dayIndex].courses = [...acc[dayIndex].courses, course].sort(
        (a, b) => a.location.courseTime.hour - b.location.courseTime.hour
      );
      return acc;
