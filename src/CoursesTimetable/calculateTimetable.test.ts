...
describe("calculateTimetable", () => {
  ...
  it("for single ongoing course returns timetable with course in given day", () => {
    ...
    const course = new BozpCourse(
      ...
      new Classroom(createCourseTime(day, 12), "random")
    );
  ...
  it("for multiple ongoing courses in same day returns timetable with courses in given day, sorted by hour", () => {
    const day = 1;
    const createCourse = (hour: number) =>
      new BozpCourse(
        1,
        "BOZP",
        true,
        new PointsRequirements(0, 20, false),
        new Classroom(createCourseTime(day, hour), "random")
      );
  ...
  it("for multiple ongoing courses in various days returns timetable with course in given days", () => {
    const createCourse = (day: DayNumber) =>
      new BozpCourse(
        1,
        "BOZP",
        true,
        new PointsRequirements(0, 20, false),
        new Classroom(createCourseTime(day, 8), "random")
      );
