...
export const calculateConflict = (
  ...
  const conflictableCourses = courses.filter(
    (otherCourse) =>
      course !== otherCourse &&
      otherCourse.accepted &&
      hasRequirements(otherCourse) &&
      !otherCourse.requirements.isCompleted
  );
  const hasTimeClash = conflictableCourses.some(
    (conflictableCourse) =>
      hasLocation(conflictableCourse) &&
      isSameTime(
        conflictableCourse.location.courseTime,
        course.location.courseTime
      )
  );
  if (hasTimeClash) {
    return {
